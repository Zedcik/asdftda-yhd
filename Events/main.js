...
message.channel.awaitMessages(filter, { max: 1, time: 1000*10, errors: ["time"] }).then(async collected => {
    ...
    verifyMsg.edit(new Discord.MessageEmbed().setDescription("Kazanan: <@!" + result.author+"> Tebrikler! **25.000** para kazandın! `.cf - .duello` komutlarını kullanarak paranı katlıyabilirsin.").setColor("2F3136")).catch(err => { return undefined; });
	verifyMsg.delete({ timeout: 15000 }); }).catch(async err => {
