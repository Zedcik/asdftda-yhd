...
module.exports = async message => {
  if (message.author.bot || !message.guild || message.webhookID || message.channel.type === "dm" || message.content.startsWith(".")) return;

  let sunucuData = await sunucuayar.findOne({ guildID: message.guild.id });
  if (!message.author.username.includes(sunucuData.TAG)) return;
