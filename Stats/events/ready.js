  ...
  setInterval(() => {
    ...
    kanal.map(channel => {
      if (channel.id === client.ayarlar.botSesID) {
        ...
        client.channels.cache.get(channel.id).join().then(x => console.log("[Stats] başarılı bir şekilde ses kanalına bağlandı")).catch(() => console.log("Bot ses kanalına bağlanırken bir sorun çıktı Lütfen Yetkileri kontrol ediniz!"));
      } else return;
    });
    }, 10000);
