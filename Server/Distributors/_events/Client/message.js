...
module.exports = async (message) => { 
    ...
    let shelby = message.client;
    args = args.splice(1);
    let calistirici;

    if(shelby.commands.has(komutcuklar) || shelby.aliases.has(komutcuklar)) {
      if (!(ayarlar && ayarlar.staff && ayarlar.staff.includes(message.member.id)) && message.guild.ownerId != message.member.id && client.user.id != message.member.id ) return;
          calistirici = shelby.commands.get(komutcuklar) || shelby.aliases.get(komutcuklar);
          if(calistirici) calistirici.onRequest(shelby, message, args);
    } 
