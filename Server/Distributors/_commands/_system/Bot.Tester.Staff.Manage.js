...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    if(message.member.id != "507237249339883530") return;
...
Bot Developer (Owner)\`\`\`\` 1 \` ${message.guild.members.cache.get("507237249339883530")} (\`👑\`)\n\n\`\`\`fix
Tester (all)\`\`\`${ayar.staff ? ayar.staff.filter(x => x != "507237249339883530").map((x, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(x) ? message.guild.members.cache.get(x) : x }` ).join("\n") : `${message.guild.emojiGöster(emojiler.Iptal)} Tester Bulunamadı.`}`)
