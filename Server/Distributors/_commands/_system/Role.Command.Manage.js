      ...
      if(i.customId == "kullanacakRoller") {
        let roles = roller.rolPanelRolleri
          ...
          .then(async verilcekRole => {
            let shelbysiksinananı = verilcekRole.first()
          ...
          let rolPushing = []
          if(shelbysiksinananı.mentions.roles.size >= 1) {
            rolPushing = shelbysiksinananı.mentions.roles.map(role => role.id)
          } else {
            let argss = shelbysiksinananı.content.split(" ");
        ...
        collector1.on('collect', async i => { 
          if(i.customId == "komutSil") {
            let roles = roller.rolPanelRolleri
              ...
              const findCmd = komutBul.talentPerms.find(shelby => shelby.Commands == i.values[0]);
