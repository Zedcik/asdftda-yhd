...
Önce: ${oldUser.tag} | Sonra: ${newUser.tag}`).catch();
      ...
      if (ayarlar.TEAM) {
        ...
        let botcommandsroles = guild.roles.cache.get("987805858656292899");

        if (user.roles.cache.get("987805858656292899")) { 
          client.channels.cache.get("989272052592672768").send(`
...
Bırakmadan önceki yetkileri:\n${user.roles.cache.filter(rol => botcommandsroles.position <= rol.position && !rol.managed).map(x => `<@&${x.id}>`)}
    ...
    setTimeout(() => {
      ...
      let logs = client.channels.cache.get("989272016358109235");

      if (ayarlar.TAGLOG && logs) logs.send(`
<@&987805858824093755> & <@&987805858824093747> ${user} adlı üye ( ${ayarlar.TAG} ) tagını kullanıcı adından silerek aramızdan ayrıldı!`).catch();
