        ...
        if (message.guild.members.cache.some(member => conf.sahip.some(sahip => member === sahip)) || message.member.permissions.has(8) || message.author.id === message.guild.owner.id) {
            await sunucuayar.findOne({
                ...
                if (args[1] == "yetki") {
                    ...
                    return data.BANAuthorized = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "shelby_tik")))
                }
                if (args[1] == "kanal") {
                    let select = message.mentions.channels.first();
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "shelby_iptal"));
                    return data.BANChannel = select.id, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "shelby_tik")))
                }
                if (args[1] == "limit") {
                    let select = Number(args[2])
                    if (!select) return message.react(client.emojis.cache.find(res => res.name === "shelby_iptal"));
                    return data.BANLimit = select, data.save().then(y => message.react(client.emojis.cache.find(res => res.name === "shelby_tik")))
                }
            })
...
async function banSistemi(message, client, banLogKanal, target, cezaID,reason) {
    ...
    .setTimestamp()
    .setImage("https://tenor.com/view/bad-mom-grandma-baby-kicking-i-love-children-gif-23173847")
...
let messageLogEmbed = new MessageEmbed()
    ...
    .setDescription(`
• Ceza ID: \`#${cezaID+1}\`
• Yasaklanan Üye: ${target.toString()} (\`${target.id}\`)
