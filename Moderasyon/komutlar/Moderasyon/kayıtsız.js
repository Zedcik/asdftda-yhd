...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (!message.member.permissions.has(128)) {
    ...
    otokayit.deleteOne({userID: target.id}).exec();
     ...
     target.roles.set(target.roles.cache.has(boost) ? [boost, ...unRegisterRol] : [...unRegisterRol]).then(x => message.react(client.emojis.cache.find(x => x.name === "shelby_tik"))).catch(y => message.react(client.emojis.cache.find(x => x.name === "shelby_iptal")));
	} else return;
}
