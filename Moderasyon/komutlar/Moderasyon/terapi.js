...
module.exports.run = async (client, message, args, durum) => {
    ...
    let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor("PINK")
    if (["Başlat", "başlat"].includes(sec)) {
        ...
        let terapiData = await stsc.findOne({ userID: message.author.id, guildID: message.guild.id }) || { Type: false };
...
2. Emoji: Çok iyiydi.\n3. Emoji: İyiydi\`\`\``))
    ...
    if (sec == "bilgi") {
        let emoji = client.emojis.cache.find(x => x.name === "shelby_tik")
