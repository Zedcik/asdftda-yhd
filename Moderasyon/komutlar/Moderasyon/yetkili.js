    ...
    if (durum) {
        if (args[0] == "top") {
		  ...
		  if (args[0] == "bak") {
			...
			.setAuthor(message.author.tag, message.author.avatarURL({dynamic: true}))
			.setDescription(`${data.filter(veri => message.guild.members.cache.get(veri.userID) && message.guild.members.cache.get(veri.userID).user.username.includes(sdata.TAG)).map((veri, index) => `<@!${veri.userID}>: ${moment(veri.Tarih).locale("tr").format("LLL")}`).join("\n")}`)
        ...
        if (target.id === message.author.id) return message.react(client.emojis.cache.find(x => x.name === "shelby_iptal"))
        ...
        await yetkiDATA.findOne({ userID: target.id, Durum: "stat" }, async (err, res) => {
            ...
            if (res.authorID != message.author.id)
                return message.reply(`Yetki aldırmaya çalıştığın üye farklı bir yetkili tarafından zaten yetkiye alınmış!`);
                    ...
                    message.react(client.emojis.cache.find(x => x.name === "shelby_tik"));
                    ...
                    hanedan.findOne({ userID: message.author.id, guildID:client.ayarlar.sunucuId }, (err, hanedanData) => {
                        ...
                        hanedan.updateOne({ userID: message.author.id, guildID: client.ayarlar.sunucuId}, { $inc: { [`Yetkili`]: 1} }, {upsert: true}).exec() });
