...
module.exports.run = async (client, message, args, durum, kanal) => {
	...
	if (message.member.permissions.has(8) || durum) {
        ...
        await ceza.deleteMany({userID: target.id}).exec().then(x => message.react(client.emojis.cache.find(x => x.name === "shelby_tik"))).catch(y => message.react(client.emojis.cache.find(x => x.name === "shelby_iptal")));
