...
if (TeamRol) {
    ...
    if (sex === "bak") {
        ...
        .addOptions(teamsex.map(team => {
                ...
                return new discordbuttons.MessageMenuOption().setValue(role ? role.id : "Rol Silinmiş").setEmoji(client.emojis.cache.find(x => x.name === "shelby_stat").id).setLabel(role ? role.name : "Rol Silinmiş")
            })
        );
