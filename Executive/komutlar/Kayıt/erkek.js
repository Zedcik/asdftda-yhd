...
message.channel.send("Kısa sürede 5 den fazla kayıt yaptığınız için yetkileriniz çekildi.")
        ...
        target.user.username.includes(tag) ? erkekRol.push(ekipRol) : erkekRol = erkekRol;
            ...
            await target.roles.remove(unreg).then(async x => {
                ...
                await message.guild.member(target.id).setNickname(`${target.user.username.includes(tag) ? tag : tag2 ? tag2 : tag} ${name} | ${age}`).then(x=> message.react(client.emojis.cache.find(res => res.name === "shelby_tik")) )
                ...
                .setDescription(`${target} kullanıcısı **Erkek** olarak kayıt edildi.`);
                await message.channel.send(embed).then(x => x.delete({timeout: 10000}))
            ...
            target.voice.setChannel(publicRooms.random().id);
