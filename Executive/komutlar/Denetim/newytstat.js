...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (durum || message.member.permissions.has(8) || message.member.roles.cache.get(sunucuData.EnAltYetkiliRol)) {
        ...
        let statemoji = client.emojis.cache.find(x => x.name === "shelby_stat");
...
let TARGETembed = new MessageEmbed().setColor("RANDOM")
...
let GENembed = new MessageEmbed().setColor("RANDOM")
.setDescription(`**${embedBas}**`)
...
let PUANembed = new MessageEmbed().setColor("RANDOM")
.setDescription(`**${embedBas}
...
let CEZAembed = new MessageEmbed().setColor("RANDOM")
.setDescription(`**${embedBas}
...
let GOREVembed = new MessageEmbed().setColor("RANDOM").setFooter("Shelby ❤️ Tori")
.setDescription(`**${embedBas}
...
let YETKIembed = new MessageEmbed().setColor("RANDOM")
.setDescription(`**${embedBas}
...
${client.emojis.cache.find(x => x.name == "shelby_deynek")} Puan Durumu
...
${client.emojis.cache.find(x => x.name == "shelby_deynek")} Yetki Atlayabilirsin!
Gerekli \`Puan\`'a ulaşarak <@&${y.ROLE_2}> yetkisine atlama hakkı kazandın!` : target.roles.cache.get(y.ROLE_1) ? `
${client.emojis.cache.find(x => x.name == "shelby_deynek")} Yetki Durumu
...
function progressBar(value, maxValue, size) {
    ...
    const progressText = `${client.emojis.cache.find(x => x.name == "shelby_ortabar")}`.repeat(progress);
    const emptyProgressText = `${client.emojis.cache.find(x => x.name == "shelby_griortabar")}`.repeat(emptyProgress);
    const bar = `${value ? client.emojis.cache.find(x => x.name == "shelby_solbar") : client.emojis.cache.find(x => x.name == "shelby_baslangicbar")}` + progressText + emptyProgressText + `${emptyProgress == 0 ? `${client.emojis.cache.find(x => x.name === "shelby_bitisbar")}` : `${client.emojis.cache.find(x => x.name === "shelby_gribitisbar")}`}`;
    return bar;
};
