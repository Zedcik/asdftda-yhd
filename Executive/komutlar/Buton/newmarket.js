...
module.exports.run = async (client, message, args, durum, kanal) => {
  ...
  .addOptions(marketopsiyonel,marketizleyici,marketdinleyici)
    let menusex = await message.channel.send('**† Witcher** Market Sistemi Aşağıda Bulunan Menülerden Market Türünü Belirtiniz!', menu)
...
.setColor("RANDOM")
.setDescription(`**† Witcher** mağazasına hoş geldin ${message.member}, Burada kendine çeşitli ödüller satın alabilirsin!`)
.addField(`† Witcher Mağaza     |   Mevcut Bakiyeniz: (31 Coin)`,`\`\`\`fix\n${izleyicitable}\`\`\``)
.addField(`† Witcher beliren butonlardan yeşil olanlara \`45 Saniye\` içerisinde tıklayarak satın alabilirsin`)
