...
module.exports.run = async (client, message, args, durum, kanal) => {
    if (message.member.permissions.has(8) || !client.ayarlar.sahip.some(x => x == message.author.id)) {
      ...
      let config = {
        "etkinlik": "989271376219226153",
        "cekilis": "987805858555637791",
    }
    ...
    let taglırold = message.guild.members.cache.filter(member => member.user.username.includes(tag) && !member.roles.cache.has(tagrol) && !member.permissions.has(8))
    let etrol = message.guild.members.cache.filter(member => !member.roles.cache.has(config.etkinlik) && !member.roles.cache.has(config.cekilis) && !member.permissions.has(8));
    let kayıtsız = message.guild.members.cache.filter(x => x.roles.cache.size === 1)
...
exports.help = {
    name: 'control',
    description: "Sunucudaki üyelere rol dağıtımı sağlar.",
    usage: 'control',
