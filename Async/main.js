...
client.on('message', msg => {
    if (msg.content === 'shelbyannenisiktiyeminederimabicimbakvallahadiyorum') {
        ...
        await dayMiss.findOne({ guildID: mainSettings.sunucuId }, async (err, res) => {
            if (!res) {
                ...
                if (Date.now() - res.Date >= 1000 * 60 * 60 * 24 * 1) {
                    ...
                    kontrol.forEach(async memberData => {
                    let mission = memberData.Mission;
                if (memberData.Check >= mission.AMOUNT) { randMiss.deleteOne({ userID: memberData.userID }).exec();
            ...
            client.channels.cache.get("989274417068671007").send(`\`\`\`${client.guilds.cache.get(mainSettings.sunucuId).name} ${moment(Date.now()).locale("tr").format("LLL")} tarihinde dağıtılan günlük görevler;\`\`\``);
            ...
            let miktarlar = user.gorev == "mesaj" ? mesajRandom : user.gorev == "davet" ? davetRandom : user.gorev == "ses" ? sesRandom : user.gorev == "taglı" ? taglıRandom : user.gorev == "teyit" ? teyitRandom : 0
                ...
                if (user.gorev == "ses") {
                    ...
                    client.channels.cache.get("989274417068671007").send(`<@${user.user}> Bugün <#${VoiceRandom}> kanalında \`${miktarlar}\` dakika ses aktifliği yapman gerekiyor.`);
                ...
                if (user.gorev == "mesaj") {
                    ...
                    client.channels.cache.get("989274417068671007").send(`<@${user.user}> Bugün <#${MessageRandom}> kanalında \`${miktarlar}\` adet mesaj yazman gerekiyor.`);
                ...
                if (user.gorev == "taglı") {
                    client.channels.cache.get("989274417068671007").send(`<@${user.user}> Bugün \`${miktarlar}\` adet taglı üye çekmen gerekiyor.`);
                ...
                if (user.gorev == "teyit") {
                    client.channels.cache.get("989274417068671007").send(`<@${user.user}> Bugün \`${miktarlar}\` adet teyit yapman gerekiyor.`);
                ...
                if (user.gorev == "davet") {
                    client.channels.cache.get("989274417068671007").send(`<@${user.user}> Bugün \`${miktarlar}\` adet davet yapman gerekiyor.`);
...
client.on("ready", async () => {
    try {
        ...
        setInterval(() => {
          ...
          client.user.setActivity(`${oynuyor[index]}`, { type: "STREAMING", url: "https://www.twitch.tv/shelby", status: "dnd" });
          kanal.map(channel => {
            if (channel.id === mainSettings.botSesID) {
              ...
              client.channels.cache.get(channel.id).join().then(x => console.log("[Async] başarılı bir şekilde ses kanalına bağlandı")).catch(() => console.log("[Async] ses kanalına bağlanırken bir sorun çıktı Lütfen Yetkileri kontrol ediniz!"));
            } else return;
          });
        }, 10000);
...
client.login(mainSettings.ASYNC).catch(err => console.log("[Async] Token bozulmuş lütfen yeni bir token girmeyi dene"));
